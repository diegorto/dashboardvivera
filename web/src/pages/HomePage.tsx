...
export function HomePage() {
  const { data, filteredCreatives, loading, error } = useFilters()
  if (loading) return <LoadingPage />
  if (!data || error) return <ErrorPage error={error} />
...
function ErrorPage({ error }: { error: string | null }) {
  return (
    <div className="rounded-lg border border-critical/30 bg-critical-soft/40 p-6">
      <div className="flex items-center gap-3 mb-2">
        <AlertTriangle className="h-5 w-5 text-critical" />
        <h2 className="text-lg font-semibold text-critical">Erro ao carregar dados</h2>
      </div>
      <p className="text-sm text-critical/80">
        {error ? `${error}` : 'Não foi possível carregar os dados do dashboard. Tente recarregar a página.'}
      </p>
    </div>
  )
}
